    ...
    extends AbstractComputerActionMojo
{
    ...
    {
        getLog().error( "Computer " + name + " failed to stop." );
    }
    ...
    {
        getLog().info( "Computer " + name + " stopped." );
    }
    ...
    {
        if ( PowerState.STOPPED.equals( c.getState() ) )
        {
            return new CompletedFuture<Success>( Success.getInstance() );
        }
        else
        {
            getLog().info( "Stopping computer " + c.getName() + "..." );
            return c.execute( new StopComputer() );
        }
    }
}