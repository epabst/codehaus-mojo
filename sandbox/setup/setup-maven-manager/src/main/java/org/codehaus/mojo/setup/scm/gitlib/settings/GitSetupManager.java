 ...
 * @since 1.0.0
 * @plexus.component role="org.codehaus.mojo.setup.SetupManager" role-hint="git"
 */
public final class GitSetupManager
    extends AbstractSetupManager
{
    private GitXpp3Reader gitReader = new GitXpp3Reader();

    private GitXpp3Writer gitWriter = new GitXpp3Writer();

    @Override
    public File getTargetFile( MavenSession session )
    {
        return GitUtil.getSettingsFile();
    }
        ...
        throws SetupMergeException
    {
        ...
        try
        {
            Settings dominantAndMergedSettings = gitReader.read( dominantReader );
            Settings recessiceSettings = gitReader.read( recessiveReader );

            SettingsUtils.merge( dominantAndMergedSettings, recessiceSettings );

            Writer writer = new StringWriter();
            gitWriter.write( writer, dominantAndMergedSettings );
