...
import org.apache.maven.scm.providers.vss.settings.io.xpp3.VssXpp3Writer;
 ...
 * @since 1.0.0
 * @plexus.component role="org.codehaus.mojo.setup.SetupManager" role-hint="vss"
 */
public final class VssSetupManager
    extends AbstractSetupManager
{
    private VssXpp3Reader vssReader = new VssXpp3Reader();

    private VssXpp3Writer vssWriter = new VssXpp3Writer();

    @Override
    public File getTargetFile( MavenSession session )
    {
        return VssCommandLineUtils.getScmConfFile();
    }
        ...
        throws SetupMergeException
    {
        ...
        try
        {
            Settings dominantAndMergedSettings = vssReader.read( dominantReader );
            Settings recessiveSettings = vssReader.read( recessiveReader );

            SettingsUtils.merge( dominantAndMergedSettings, recessiveSettings );

            Writer writer = new StringWriter();
            vssWriter.write( writer, dominantAndMergedSettings );
